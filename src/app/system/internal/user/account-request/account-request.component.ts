...
export class AccountRequestComponent implements OnInit {
  ...
  public title : string = 'Account Request Entries';
  ...
  ngOnInit() : void {
     ...
     this.route.queryParamMap.subscribe((params : ParamMap) => {
      ...
      if (Object.keys(this.searchQuery).length) paramsObject = {...paramsObject , ...this.searchQuery };
        ...
        this.getAcctRequestUsers(paramsObject);
    ...
    .subscribe((users : User[]) => { 
      ...
      if (users.length < 1) { this.usersLength = users.length;
        ...
        this.error = Object.assign({'resource' : 'User Entry or Entries'} , this.ems.message);  
  ...
  public getAcctRequestUsers(sq : SearchQuery) {

   this.us.getAcctRequestUsers(sq)
  ...
  public deleteManyUser() : void | boolean { let confirmDeletion = confirm('Are you sure you want to perform this action?');
      ...
      if (confirmDeletion) {
        ...
        .subscribe((val) => { this.users = this.users.filter((user) => {
          ...
          this.ns.addNotification('Operation is successful and many account request is deleted.'); 
          ...
          this.ns.addNotification('Operation is unsuccessful and many account request is not deleted.');   } )  }
