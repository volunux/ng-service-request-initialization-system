...
@Component({
  ...
  'selector' : 'app-general-thesis-entries',

  'templateUrl' : './general-thesis-entries.component.html',

  'styleUrls' : ['./general-thesis-entries.component.css'] ,
...
export class GeneralThesisEntriesComponent implements OnInit {

  constructor(private route : ActivatedRoute , private router : Router , private location : Location , private gtfs : GeneralThesisFormService ,

              private gts : GeneralThesisService , private grss : GeneralSearchService , private ems : ErrorMessagesService , private ns : NotificationService ) { 
  ...
  public trackByName(idx : number , entry : GeneralThesis) : string {
  ...
  ngOnInit() : void {
    ...
    this.searchCtrl = this.gtfs.searchForm(this.searchFilters);
    ...
    this.searchTerm.pipe(
      ...
      switchMap((query : SearchQuery) => { return this.grss.searchTerms<GeneralThesis[]>(query , this.$link , `${this.systemType} Entries` , []); })  )

    .subscribe(($entries : GeneralThesis[]) => { 
  ...
  if (checked) { if (gridx) this.esdl.push(gridx);  }
    ...
    public deleteManyEntry() : void | boolean { let confirmDeletion = confirm('Are you sure you want to perform this action?');
      ...
      if (confirmDeletion) {
        ...
        this.gts.$deleteManyEntry(this.esdl)
