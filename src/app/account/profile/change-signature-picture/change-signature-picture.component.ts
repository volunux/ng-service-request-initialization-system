...
import { HttpEvent , HttpResponseBase , HttpEventType } from '@angular/common/http';

import { of , Observable , Subject , Subscription } from 'rxjs';

@Component({

  'selector' : 'app-change-signature-picture',

  'templateUrl' : './change-signature-picture.component.html',

  'styleUrls' : ['./change-signature-picture.component.css']

})

export class ChangeSignaturePictureComponent implements OnInit , AfterViewInit , OnDestroy {
  ...
  public deleteUpload($evt) : void {
			...
			let displayErr = setTimeout(() => {
						...
						this.$setText(this.fileUploadError1 , 'An error has occured while removing image. Please try again.' , 'block');
  ...
  public addFile(e , file) : any {
  	...
  	this.$clearText([this.fileUploadMessage1 , this.fileUploadError1 , this.fileUploadError11 , this.fileUploadProgress1 , this.fileUploadText1]);
		...
		let data = { 'photo' : file.files[0] };
          ...
          if (hash instanceof Object && hash.noHash) {
					...
					else  if (hash instanceof Object && !(hash instanceof HttpResponseBase)) {
      ...
      if (hash instanceof Object && hash.status == 201) {
		...
		if (hash instanceof Object && !(hash instanceof HttpResponseBase)) {
